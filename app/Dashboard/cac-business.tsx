...
const BusinessNameRegistration = () => {
  ...
  const handleSubmit = async () => {
    // Basic Validation
    if (!sname || !proprietorPhone || !proprietorEmail || !businessAddress || !natureOfBusiness || !proposedName1 || !proposedName2 || !proprietorAddress) {
      showAlert("Validation Error", "Please fill in all text fields.");
      return;
    }

    if (!passport || !signature || !ninImage) {
      showAlert("Validation Error", "Please upload all required images and signature.");
      return;
    }
    ...
    try {
      ...
      formData.append("proprietor_passport", {
        uri: Platform.OS === 'android' ? passportUri : passportUri.replace('file://', ''),
        name: passportName,
        type: passportType,
      });
      ...
      if (responseData.success) {
        router.replace({
          pathname: "/Dashboard/cac-success",
          params: {
            sname: sname,
        ...
        >
          ...
          <View style={styles.content}>
            <View style={[styles.noteBubble, { backgroundColor: isDark ? "#3d1b1b" : "#ffe2e2", borderColor: isDark ? "#b00020" : "#f0a4a4" }]}>
              ...
              <Text style={[styles.noteText, { color: isDark ? "#ffcdd2" : "#4a1f1f" }]}>This Business Name Registration will cost you <Text style={[styles.noteAmount, { color: isDark ? "#ff5252" : "#b00020" }]}>N19000</Text></Text>
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Surname (Person Registering)</Text>
            <TextInput
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Proposed Name Option 1</Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.surface, borderColor: colors.inputBorder, color: colors.text }]}
              placeholderTextColor={colors.textMuted}
              value={proposedName1}
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Proposed Name Option 2</Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.surface, borderColor: colors.inputBorder, color: colors.text }]}
              placeholderTextColor={colors.textMuted}
              value={proposedName2}
            ...
            <Text style={[styles.sectionTitle, { color: colors.text, marginTop: 10 }]}>Proprietor Details</Text>

            <Text style={[styles.inputLabel, { color: colors.text }]}>Proprietor Address</Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.surface, borderColor: colors.inputBorder, color: colors.text }]}
              placeholderTextColor={colors.textMuted}
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Proprietor Phone Number</Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.surface, borderColor: colors.inputBorder, color: colors.text }]}
              placeholderTextColor={colors.textMuted}
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Proprietor Email</Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.surface, borderColor: colors.inputBorder, color: colors.text }]}
              placeholderTextColor={colors.textMuted}
            ...
            <Text style={[styles.sectionTitle, { color: colors.text, marginTop: 10 }]}>Uploads & Signature</Text>
            ...
            <View style={styles.uploadRow}>
              ...
              >
                {ninImage ? (
                  <Image source={{ uri: ninImage.uri }} style={styles.previewImage} />
                ) : (
                  <>
            ...
            >
              {signature ? (
                <Image source={{ uri: signature }} style={styles.previewSignature} resizeMode="contain" />
              ) : (
                <>
                  <Text style={[styles.uploadLabelText, { color: colors.text }]}>Owner's Signature</Text>
                  <Text style={[styles.uploadSubText, { color: colors.textMuted }]}>Tap to draw signature (Blue Color)</Text>
                </>
              )}
            </TouchableOpacity>
      ...
      >
        <View style={[styles.sigContainer, { backgroundColor: colors.background }]}>
          <View style={styles.sigHeader}>
            <Text style={[styles.sigTitle, { color: colors.text }]}>Draw Signature (Blue Color)</Text>
