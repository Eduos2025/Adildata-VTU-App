    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <Tabs
        ...
        tabBar={({ state, navigation }) =>
          ...
          state.routes[state.index]?.name === "notification-detail-screen" ||
          state.routes[state.index]?.name === "referral-screen" ? null : (
            <View
              style={[
                styles.bottomNav,
                {
              ...
              {state.routes
                .filter(
                  (route) =>
                    ...
                    route.name !== "notification-detail-screen" &&
                    route.name !== "referral-screen",
                )
                .map((route) => {
                  const isFocused =
