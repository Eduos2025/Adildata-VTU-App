...
const Electricity = () => {
  ...
  const validateMeter = async (meter: string, serviceID: string, type: string) => {
    ...
    try {
      ...
      const response = await fetch("https://api.rahausub.com.ng/validateMeter.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          meter,
  ...
  const goToResult = (success: boolean, token?: string) => {
    ...
    router.replace({
      pathname: success
        ? "/Dashboard/electricity-success"
        : "/Dashboard/electricity-failed",
      params,
    });
  };
  ...
  const handleElectricityPurchase = async (finalPin: string) => {
    try {
      ...
      const response = await fetch("https://api.rahausub.com.ng/purchaseElectricity.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token: userToken,
