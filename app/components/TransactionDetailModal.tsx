...
const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const rawToken = transaction.token || receipt.token || receipt.Purchased_Code || receipt.purchased_code || "";
  const cleanToken = String(rawToken).replace(/\D/g, '');

  // 2. Identify essential details for display (whitelist approach)
  const whitelist = [
    "meter_number", "meterNumber", "customer_name", "customerName", "address",
    "distributor", "meter_type", "meterType", "network", "plan", "data_plan", "type"
  ];
  ...
  const entries = Object.entries(details).filter(([key, val]) => {
    const isWhitelisted = whitelist.some(w => w.toLowerCase() === key.toLowerCase());
    const isDuplicateToken = key.toLowerCase() === "token" || key.toLowerCase() === "purchased_code";
    return isWhitelisted && !isDuplicateToken && val !== null && val !== undefined && val !== "";
  });

  const status = transaction.status || receipt.status || "Completed";
  const isSuccess = transaction.subtitle?.toLowerCase().includes("successfully") || transaction.status?.toLowerCase() === "success";
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <View style={styles.receiptHeader}>
            <View style={[styles.iconBadge, { backgroundColor: isFailed ? "#fff1f1" : "#f0fff4" }]}>
              <Ionicons
                name={isFailed ? "close-circle" : (transaction.negative ? "arrow-up-circle" : "arrow-down-circle")}
                size={40}
                color={isFailed ? "#d14343" : "#20a85b"}
              />
            </View>
            <Text style={[styles.title, { color: colors.text }]}>{transaction.title}</Text>
            <Text style={[styles.amount, { color: isFailed ? "#d14343" : "#20a85b" }]}>
              {transaction.amount}
            </Text>
          ...
          {cleanToken ? (
            <View style={[styles.tokenCard, { backgroundColor: isDark ? colors.surface : "#f0f9ff", borderColor: colors.primary }]}>
              <Text style={[styles.tokenLabel, { color: colors.textMuted }]}>ELECTRICITY TOKEN</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                <Text style={[styles.tokenValue, { color: colors.primary, marginBottom: 0 }]}>{cleanToken}</Text>
                <TouchableOpacity onPress={() => copyToClipboard(cleanToken, "Token")}>
          ...
          <View style={styles.detailsContainer}>
            ...
            {transaction.phone && (
              <View style={styles.detailRow}>
                ...
                <Text style={[styles.detailValue, { color: colors.text }]}>{transaction.phone}</Text>
              </View>
            )}
            ...
            {entries.length > 0 ? (
              entries.map(([key, value]: [string, any], index) => (
                <View key={key} style={styles.detailRow}>
                  ...
                  <TouchableOpacity
                    activeOpacity={0.7}
                    onLongPress={() => copyToClipboard(String(value), key.replace(/_/g, ' '))}
                    style={{ flex: 2 }}
                  >
              ...
              !cleanToken && (
                <View style={styles.detailRow}>
                  <Text style={[styles.detailLabel, { color: colors.textMuted }]}>DETAILS</Text>
